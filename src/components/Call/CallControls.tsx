...
const CallControls = ({ isStreamHidden, onToggleStreamVisibility }: CallControlsProps) => {
  ...
  return (
    <div className="flex justify-center items-center space-x-6 md:space-x-3">
      ...
      <button
        onClick={toggleMic}
        className={`w-14 h-14 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors duration-200 cursor-pointer ${
          isMicMuted
      ...
      >
        {isMicMuted ? (
          ...
          <svg className="w-7 h-7 md:w-5 md:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        className="w-16 h-16 md:w-12 md:h-12 bg-red-500 hover:bg-red-600 active:bg-red-700 rounded-full flex items-center justify-center transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 cursor-pointer hover:ring-2 hover:ring-red-400/50"
      >
      ...
      <button
        ...
        className={`w-14 h-14 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors duration-200 ${
      ...
      {hasAnyScreenStream && (
        ...
        >
          {isStreamHidden ? (
            <Eye className="w-7 h-7 md:w-5 md:h-5 text-white" />
          ) : (
            <EyeOff className="w-7 h-7 md:w-5 md:h-5 text-white" />
          )}
        </button>
      )}
