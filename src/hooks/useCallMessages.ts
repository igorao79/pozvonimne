...
export const useCallMessages = ({ chatId, userId }: UseCallMessagesProps) => {
  ...
  const updateCallMessage = async (callChatId: string, callerId: string, newStatus: string, duration: number = 0) => {
    try {
      console.log('📞 Обновление сообщения о звонке:', { callChatId, callerId, newStatus, duration })
  ...
  useEffect(() => {
    ...
    // 1. Начало звонка (кто-то начал звонить)
    ...
    // 4. Звонок отклонен (только если был входящий звонок)
    else if (!isReceivingCall && prevState.isReceivingCall && !isCallActive) {
    ...
    // 5. Пропущенный звонок (если звонили, но не приняли)
    else if (!isReceivingCall && prevState.isReceivingCall && !prevState.isCallActive && !isCallActive) {
      console.log('📞 Пропущенный звонок - обновляем статус на missed')
      updateCallMessage(callChatId, currentCallerId, 'missed')
      callMessageIdRef.current = null
    }

    // 6. Отмена исходящего звонка (пользователь сам отменил звонок)
